...
class PPU {
    constructor(dmg, upscaleFactor = 2) {
        this.dmg = dmg;
        this.mmu = dmg.mmu;
    ...
    setQuality(scale) {
        const nextScale = Math.max(1, Math.min(3, parseInt(scale, 10) || 1));
        if (nextScale === this.upscaleFactor) {
            return;
        }

        this.upscaleFactor = nextScale;
        for (const canvas of this.canvasList) {
            canvas.width = 160 * this.upscaleFactor;
            canvas.height = 144 * this.upscaleFactor;
        }
...
class TetrisPPU extends PPU {
    constructor(dmg) {
        super(dmg, 2);
        this.remakeEnabled = false;
        this.remakeLoaded = false;
        this.remakeTileMap = new Map();
        this.backgroundImage = "url('remake/TETRIS/bg.png')";

        fetch("remake/TETRIS/tilemap.json")
            .then(response => {
                if (!response.ok) {
                    throw new Error("Tetris remake tilemap not found");
                }
                return response.json();
            })
            .then(data => {
                this.remakeData = data;
                data.map.forEach((tileCode, index) => {
                    this.remakeTileMap.set(tileCode, index);
                });

                this.tilesImage = new Image();
                this.tilesImage.onload = () => {
                    this.remakeLoaded = true;
                };
                this.tilesImage.src = "remake/TETRIS/tiles.png";
            })
            .catch(() => {
                this.remakeLoaded = false;
            });
    ...
    toggleRemake() {
        this.remakeEnabled = !this.remakeEnabled;
        document.getElementById("screen-bg").style.backgroundImage = this.remakeEnabled ? this.backgroundImage : "none";
        document.getElementById("screen-bg").style.backgroundSize = this.remakeEnabled ? "cover" : "";
        document.getElementById("screen-bg").style.backgroundPosition = this.remakeEnabled ? "center" : "";
        document.getElementById("screen-bg").style.backgroundColor = this.remakeEnabled ? "#000000" : "";
    }
    ...
    drawLine(ly) {
        ...
        if (!this.remakeEnabled || !this.remakeLoaded) {
            return;
        }
        ...
        const _ff40 = this.mmu.memory[0xff40];
        const bgTileMapOffset = _ff40 & 0x08 ? 0x9c00 : 0x9800;
        const _ff43 = this.mmu.memory[0xff43];
        const y = ly >> 3;
        const x0 = _ff43 >> 3;
        ...
        for (let x = x0; x <= x0 + 20; x++) {
            ...
            this.drawTile(tileOffset, tileX, ly, this.contextList[2]);
        }
    }

    drawTile(tileOffset, x, y, context) {
        const tileCode = [...this.mmu.memory.slice(tileOffset, tileOffset + 16)]
            .map(value => value.toString(16).padStart(2, "0"))
            .join("");
        const remakeIndex = this.remakeTileMap.get(tileCode);
        if (remakeIndex === undefined) {
            return;
        }
